...
class MerkleZKPService {
  ...
  async verifyUser(userId, nik, nama, ttl, key) {
    ...
    if (!this.currentMerkleTree?.leaves?.length) {
      ...
      console.log("🔄 Rebuilt Merkle tree root:", newRoot);
    }
    ...
    console.log("🔹 proofRoot:", publicSignals[0]);
    console.log("🔹 contractRoot:", contractRootBig.toString());
    ...
    // 6️⃣ Submit ke kontrak
    const leafHex = toBytes32Hex(await computeLeaf(userData.identityHash, userData.salt));
    console.log("🚀 Sending proof to contract...");

    try {
      const tx = await merkleZkpContract.verifyIdentity(proof.a, proof.b, proof.c, leafHex);
      const receipt = await tx.wait();

      // 7️⃣ Update status user
